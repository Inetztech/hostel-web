...
//   deleteHostelWithAdmin, fetchTenants, fetchBranches, fetchAllPages,
...
//   Wallet, FileText, BedDouble, DoorOpen, User as UserIcon,
// } from "lucide-react";
...
//   bedPrice: number;
//   /** capacityBeds * bedPrice — the total price for this hostel. */
//   totalPrice: number;
//   status: string;
//   city: string;
// };
...
//  *  types a bed count / price per bed — totalBeds * bedPrice. */
// function TotalPricePreview({ totalBeds, bedPrice }: {
//   totalBeds: string; bedPrice: string;
// }) {
...
//   const [editForm, setEditForm] = useState<CombinedForm>(EMPTY_FORM);
...
//       capacityBeds,
//       bedPrice,
//       totalPrice: capacityBeds * bedPrice,
...
//   // Includes bed capacity, actual bed count, price per bed, branch count,
//   // and computed total price. "Total Beds" here reports the REAL bed count
//   // (h.totalBeds, actual Bed rows created) rather than the declared
//   // capacity — "Bed Capacity" is exported as its own separate column so
...
//     }
//     const headers = [
//       "Hostel Name", "Address", "City", "Price/Bed", "Bed Capacity", "Total Beds", "Total Price",
...
//     // Price per bed is now REQUIRED — must be a positive number.
...
//     if (!form.adminPassword.trim()) { toast.error("Admin password required"); return; }
//     setSubmitting(true);
//     try {
...
//         // Blank = keep current password, per backend contract.
...
//         .h-form-section { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #94a3b8; margin: 14px 0 6px; }
...
//               <DropdownMenu>
//                 <DropdownMenuTrigger asChild>
//                   <button type="button" className="h-page-btn" style={{ width: "auto", padding: "0 10px" }}>
...
//                 </span>
//               </div>
//               {/* Total Price = capacityBeds * bedPrice. Shows "—" only
//                   when no price is set (legacy records) — set one via
//                   Edit to populate it. */}
//               <div className="h-view-row">
...
//       {/* Add Dialog — Hostel + Admin combined */}
...
//             />
//             <TotalPricePreview totalBeds={form.totalBeds} bedPrice={form.bedPrice} />

//             <div>
//               <label className="h-form-section" style={{ display: "block", marginBottom: 6 }}>Document</label>
//               <Input
//                 type="file"
...
//               />
//               <TotalPricePreview totalBeds={editForm.totalBeds} bedPrice={editForm.bedPrice} />

//               <div>
//                 <label className="h-form-section" style={{ display: "block", marginBottom: 6 }}>
...
//               <Input placeholder="New Password (leave blank to keep current)" type="password" value={editForm.adminPassword} onChange={(e) => setEditForm({ ...editForm, adminPassword: e.target.value })} />
//             </div>
//           )}
//           <DialogFooter>
