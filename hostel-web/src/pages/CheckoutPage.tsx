...
// import { Room, Tenant, Rent, DEFAULT_EB_RATE, MONTHS } from "@/lib/types";
...
//     (sum, r) => sum + (r.pendingAmount ?? r.rentAmount ?? 0),
//     0
//   );

//   const totalEBDue = pendingRents.reduce(
//     (sum, r) =>
//       sum + (r.paymentStatus === "PARTIAL" ? 0 : r.ebAmount ?? 0),
//     0
//   );
...
//                           ₹{Math.round(rent.pendingAmount ?? 0).toLocaleString()}
//                         </span>
//                       </>
//                     )}
...
//         {/* Summary Footer — shows totals only, NOT net payable */}
//         {pendingRents.length > 0 && (
//           <>
...
//                 <span>Total Due</span>
//                 <span>₹{Math.round(totalRentDue + totalEBDue).toLocaleString()}</span>
//               </div>
//             </div>
//           </>
//         )}
...
//       const allReadings = await getEBReadings();
...
//         if (roomAcReadings.length > 0) {
//           setAcPrev(String(roomAcReadings[0].acCurrentReading ?? 0));
//         } else {
//           setAcPrev(String(selected.acJoinReading ?? 0));
//         }
//       }
...
//         setFinalPrev(String(roomReadings[0].currentReading));
//       } else {
...
//       const ebBills = room?.flatId
//         ? await getTenantWiseEBBill({ flatId: Number(room.flatId) })
//         : await getTenantWiseEBBill({ roomId: Number(room.id) });
...
//       if (
//         (checkout?.pendingRents ?? []).some((r) => r.paymentStatus !== "PAID")
//       ) {
...
//             <table>
//               <tr><td>Pending Rent</td><td class="right">₹${summary.totalRentDue.toLocaleString()}</td></tr>
//               <tr><td>Pending EB</td><td class="right">₹${tenantEBDue.toLocaleString()}</td></tr>
//               <tr><td>Extra Rent</td><td class="right">₹${extraRent.toLocaleString()}</td></tr>
//               <tr><td>Extra EB</td><td class="right">₹${finalEBAmount.toLocaleString()}</td></tr>
...
//                 <td class="right">₹${Math.ceil(calculatedNetPayable).toLocaleString()}</td>
//               </tr>
//             </table>
//           </div>
//           <br/><br/>
...
//                     {/* ── Clickable Pending Rent Records row — plain blue text, no background ── */}
//                     <div className="flex justify-between items-center">
...
//                         onKeyDown={(e) =>
//                           e.key === "Enter" && setPendingRentDialogOpen(true)
//                         }
//                         style={{
//                           display: "inline-flex",
//                           alignItems: "center",
...
//                           userSelect: "none",
//                         }}
//                         title="View pending rent details"
//                       >
...
//                         }`}
//                       >
//                         ₹{Math.ceil(calculatedNetPayable).toLocaleString()}
//                       </span>
//                     </div>
//                   </CardContent>
//                 </Card>
...
//                       >
//                         <LogOut className="h-4 w-4 mr-2" /> Proceed to
//                         Check-Out
//                       </Button>
//                       <AlertDialogContent>
//                         <AlertDialogHeader>
...
// /**
//  * For a PARTIAL record the backend's pendingAmount = totalAmount - paidAmount,
//  * which bundles BOTH the rent portion AND the EB portion still outstanding.
//  *
//  * We need to split them cleanly so the Settlement Summary shows:
//  *   Total Pending Rent Due  → only the rent portion outstanding
//  *   Total Pending EB Due    → only the EB portion outstanding
//  *
//  * Logic per record:
//  *   PAID    → rent pending = 0,  EB pending = 0
//  *   PENDING → rent pending = rentAmount,  EB pending = ebAmount
//  *   PARTIAL → paid covers rent first, then EB
//  *             rentPending = max(rentAmount - paidAmount, 0)
//  *             ebPending   = max(ebAmount  - max(paidAmount - rentAmount, 0), 0)
//  */
...
//   // Use the same split helper so dialog totals match Settlement Summary
//   const totalRentDue = pendingRents.reduce(
...
//         // ── Split pendingAmount into rent-only and EB-only portions ──
//         // This correctly sums ALL months (May ₹412 + June ₹390 = ₹802)
//         const totalRentDue = unpaidRents.reduce(
//           (sum, r) => sum + splitPending(r).rentPending,
//           0
//         );
...
//         // ✅ FIX: Use splitPending total which sums ALL months correctly.
//         // Do NOT override with getTenantWiseEBBill — that only returns
//         // the current month's EB and would discard older pending EB amounts.
//         setTenantEBDue(totalEBDue);
//       }
...
//       // NOTE: getTenantWiseEBBill intentionally removed here.
//       // It was overriding tenantEBDue with only the latest month's EB
//       // (e.g. ₹390 for June), causing older pending EB (e.g. ₹412 for May)
//       // to be silently dropped. The splitPending logic above is correct.
...
//   // ── Net payable: rent pending + EB pending + extras + maintenance - advance ──
//   const calculatedNetPayable = summary
//     ? summary.totalRentDue +
//       tenantEBDue +
//       extraRent +
...
//                     {/* Clickable Pending Rent Records row */}
//                     <div className="flex justify-between items-center">
//                       <span className="text-muted-foreground">
//                         Pending Rent Records
//                       </span>
