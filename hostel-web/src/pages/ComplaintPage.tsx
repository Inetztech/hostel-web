...
//     const [data, branchData, roomData] = await Promise.all([
//     role === "TENANT"
//       ? getMyComplaints()
//       : getAllComplaints(),
//     getBranches(0, 50),
...
// }, [complaints, selectedBranch, rooms]);
...
//     </Select>
//   ) : role === "TENANT" ? null : (
//     /* WARDEN and other roles: static label */
...
const ComplaintPage = () => {
  ...
  const loadComplaints = useCallback(async () => {
    try {
      ...
      const [data, branchList, roomList] = await Promise.all([
        role === "TENANT" ? getMyComplaints() : getAllComplaints(),
        fetchAllPages<Branch>(getBranches),
        fetchAllPages<Room>(getRooms),
      ]);

      // ── FIX: both getBranches and getRooms return { content, totalElements }
      //         so we must extract .content before storing in state ──
      setComplaints(toArray<Complaint>(data));
      setBranches(toArray<Branch>(branchList));
      setRooms(toArray<Room>(roomList));
    } catch (err) {
      console.error(err);
      toast.error("Failed to load complaints");
    } finally {
